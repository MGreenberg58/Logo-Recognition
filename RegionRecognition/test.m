...
for i = 1:numTesting
    ...
    img = imread(file);
    [boxes, scores] = detect(trainedDetector, img);

    if numel(scores) == 1
        imgs = imcrop(img, boxes);
    else
        for j = 1:4:numel(boxes)
            imgs = imcrop(img, boxes(j, 1:4));
        end
    end

    [YPred, score] = classify(netTransfer, imresize(imgs, [227, 227]));
...
good = numel(predicted(predicted == actual));
